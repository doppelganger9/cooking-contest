<script>
  import InputRangeComponent from './input-range.component.svelte';

  import { evaluating, evaluations, contest } from '../evaluations/evaluations.store.js';
  ...
  function doneEvaluating() {
    evaluations.update( list => {
      ...
      saveEvaluation(db, $username, list[currentEvaluationIndex]).then(() => {
        // ok
      }).catch(err => {
        // error
      });
...
<section>
  ...
  <br/>
    ...
    <InputRangeComponent name="criteria{indexCriterion}" id="criteria{indexCriterion}" min={1} max={5} step={.5} bind:value={$evaluations[currentEvaluationIndex].ratings[indexCriterion]} />
    <br/>
    {/each}
